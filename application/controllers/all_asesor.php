...
class all_asesor extends CI_Controller {
	/**
	 * Constructor
	 */
	...
	function get_all($offset)
	{
		...
		if ($num_rows > 0)
		{
			...
			$fields = array();
			array_push($fields, 'No','Nama Peserta' , 'No Registrasi', 'Pendidikan' , 'Email' , 'No Telephone', 
								'Alamat', 'Surat Pernyataan', 'Action');
			$this->table->set_heading($fields);
			$data_asesor = $this->asesor_model->getAllData();
			
			$i = 0;

			foreach ($data_asesor as $row)
			{
		...
		$data['link'] = array('link_add' => anchor('all_asesor/add/','tambah data', array('class' => 'add'))
								);
	...
	function add_process()
	{
		...
		$data['h2_title'] 		= 'Data Asesor > Tambah Data';
		$data['main_view'] 		= 'asesor/asesor_form';
		$data['form_action']		= site_url('all_asesor/add_process');
		$data['link'] 			= array('link_back' => anchor('all_asesor','kembali', array('class' => 'back'))
										);
	...
	function update($id_asesor)
	{
		...
		$data['main_view'] 		= 'asesor/asesor_form';
		$data['form_action']		= site_url('all_asesor/update_process');
