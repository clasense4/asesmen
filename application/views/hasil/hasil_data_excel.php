...
$objPHPExcel->getActiveSheet()->setTitle('Kegiatam');
...
$objPHPExcel->getActiveSheet()->setCellValue('A1', 'LIST KEGIATAN');

$objPHPExcel->getActiveSheet()->setCellValue('A3', 'No');
$objPHPExcel->getActiveSheet()->setCellValue('B3', 'Nama');
$objPHPExcel->getActiveSheet()->setCellValue('C3', 'Instansi');
$objPHPExcel->getActiveSheet()->setCellValue('D3', 'Tanggal');
$objPHPExcel->getActiveSheet()->setCellValue('E3', 'Mulai Proyek');
$objPHPExcel->getActiveSheet()->setCellValue('F3', 'Selesai Proyek');
...
if($kegiatan_list == TRUE) :
    foreach ($kegiatan_list as $row) :
        $no++;
        $index =  3 + $no;
        
        $objPHPExcel->getActiveSheet()->setCellValue('A'. $index, $no);
        $objPHPExcel->getActiveSheet()->setCellValue('B'. $index, $row->nama);
        $objPHPExcel->getActiveSheet()->setCellValue('C'. $index, $row->instansi);
        $objPHPExcel->getActiveSheet()->setCellValue('D'. $index, $row->tanggal);
        $objPHPExcel->getActiveSheet()->setCellValue('E'. $index, $row->proyek_mulai);
        $objPHPExcel->getActiveSheet()->setCellValue('F'. $index, $row->proyek_selesai);
